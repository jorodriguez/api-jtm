...
const uploadEjercicio = async(request, response) => {
    console.log("@uploadEjercicio");
    try {

        const data = { id_usuario, genero } = request.body;
        const image = request.file.buffer;
        //      let metadataUpload = await uploadService.uploadEjercicio(id_alumno, genero, image);
