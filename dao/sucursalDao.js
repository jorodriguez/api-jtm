...
const getSucursalPorEmpresa = async(idEmpresa) => {
    ...
    return await genericDao.findAll("SELECT id,nombre,direccion,class_color,co_empresa,telefono,nombre_folder FROM co_sucursal WHERE co_empresa =  $1 and eliminado = false order by nombre", [idEmpresa]);
};

const getSucursalPorId = async(idSucursal) => {
    console.log("@getSucursalPorId");

    return await genericDao.findOne(`SELECT suc.id,
	                                        suc.nombre,suc.direccion,suc.class_color,suc.co_empresa,suc.telefono,
	                                        suc.nombre_folder as folder_sucursal, empresa.nombre_folder as folder_empresa
                                    FROM co_sucursal suc inner join co_empresa empresa on empresa.id = suc.co_empresa
                                    WHERE suc.id =  $1 and suc.eliminado = false`, [idSucursal]);
